...
public class Module2_Task5_thru_8_IT extends Mockito {
	...
	public void setUp() throws Exception {
		try {
			method = Whitebox.getMethod(CartController.class, "deleteFromCart", HttpServletRequest.class,
		...
		if (method != null) {
			...
			try {
				...
				try {
					PowerMockito.verifyPrivate(cartController).invoke("deleteFromCart", request, response);
	...
	private void checkMethodExists() {
		errorMsg = "private void deleteFromCart() does not exist in CartController";
		assertNotNull(errorMsg, method);
		errorMsg = "After action \"" + "/delete" + "\", did not call deleteFromCart().";
	...
	// } catch (Throwable e) {}
	// errorMsg = "Does not call shoppingCart.deleteCartItem() in
	// deleteFromCart().";
